...
\Procedure{K-MEANS}{k: clusters, D: conjunto de dados}
	...
	\Repeat 
		\State {(re) atribuir cada objeto ao cluster ao qual o objeto tem menor distância;} 
		\State {atualizar o centroide do cluster;} 
...
\Procedure{DBSCAN}{\textit{MinPts}: limiar da vizinhança,  D: conjunto de dados, $ \varepsilon $ : raio  }
	...
	\Repeat
		\State {escolher um objeto \textit{x} não selecionado;} 
		\State {actualizar objeto \textit{x} como selecionado;} 
		\If {a $ \varepsilon$-vizinhança de \textit{x} tem pelo menos \textit{MinPts} objetos}
			\State {criar um novo \textit{cluster C} e adicionar objeto \textit{x} ao \textit{cluster C};}
			\State {Seja N o conjunto de objetos na $\varepsilon$-vizinhança de \textit{x}; }
			\For {cada ponto \textit{x'} em N}
				\If {\textit{x'} não selecionado}
					\State {Marcar \textit{x'} como selecionado}
					\If {a $\varepsilon$-vizinhança de \textit{x'} tem pelo menos \textit{MinPts}}
						\State {adicionar ponto a N}
					\EndIf
				\EndIf
				\If {\textit{x'} não pertence a nenhum cluster}
					\State {adicionar \textit{x'} a \textit{C}}
				\EndIf
			\EndFor
			\State {\textbf{output} C;}
		\Else { marcar \textit{x} como \textit{outlier}}
		\EndIf
...
Os atributos binários podem ser divididos em dois tipos  diferentes, os simétricos e os assimétricos, sendo em seguida apresentadas as funções de distância para ambos os casos~\citet{Liu2011}.
...
%, como representado na Figura~\ref{fig:temptweep1}. 
\begin{figure}[h]
\centering
...
Como vimos na Secção~\ref{subsec:tpresult}, a ferramenta TweeProfiles permite uma grande variedade de combinações que produzem resultados que podem variar no número e posição geográfica dos \textit{clusters} consoante a influência e peso das dimensões em consideração. 
...
Conforme dito anteriormente (Capítulo~\ref{chap:intro}), o objetivo deste projeto é estender o TweeProfiles para identificar padrões também nas imagens publicadas nos tweets, para além das dimensões já analisadas. Assim, na próxima secção, são apresentadas abordagens para a representação de informação visual, que servirá de base a este projeto.
% De forma a dar continuidade ao projeto desenvolvido e apresentar uma alternativa de conjugação de dimensões para a descoberta de padrões em tweets, nesta dissertação serão utilizadas da mesma forma, as dimensões espaço-temporal, e será substituído o conteúdo textual pelo visual. No caso da dimensão social, esta não será incluída por não apresentar a informação de forma clara para um utilizador, sendo necessário um estudo mais aprofundado sobre esta temática, que não faz parte dos objetivos desta dissertação.
  

%Assim, conclui-se que a utilização das dimensões espaço-temporal fazem todo o sentido no desenvolvimento desta dissertação. No caso do conteúdo textual será substituído pelo conteúdo visual. A dimensão social, também não será incluída por esta não apresentar a informação de forma clara para um utilizador, sendo necessário um estudo mais aprofundado sobre esta temática, que não faz parte dos objetivos desta dissertação.
...
%
%A estrutura do descritor GoF / GoP é idêntica à do cor escalável, com a exceção do campo agregação, que especifica como os  da cor de diferentes imagens/\textit{frames} foram combinadas antes da extração do histograma de cor. Os valores possíveis são média, mediana e cruzamento. 
%
...
%No caso do filtro de diferença Gaussianas \textit{DoG} (\textit{"Diference of Gaussians"}) descrito na equação~\ref{eq:dog}, este é dado pela diferença entre as imagens filtradas em escalas diferentes mas próximas, separadas por uma constante \textit{k} e pode ser definida como:
%
%\begin{equation}
...
Este filtro \textit{DoG} aplicado a uma imagem provoca uma perda de nitidez (efeito desfoque) e torna-se assim capaz de detetar variações de intensidade nas imagens, como por exemplo, nos contornos. A Figura~\ref{fig:siftdog} ilustra este processo de obtenção da diferença de Gaussianos. % e consequente formação das oitavas. 
...
Por fim é realizado o processo de deteção de extremos, onde um píxel é comparado com os seus oito vizinhos na imagem atual e com os nove vizinhos das imagens de escalas adjacentes, numa região de 3x3. A figura~\ref{fig:siftdog2} ilustra este processo. 

\begin{figure}[h]
\centering
\includegraphics[width=0.4\linewidth]{./figures/sift_dog2.png}
\caption{Ilustração do processo de deteção de máximos e mínimos das imagens de Diferença Gaussiana. O píxel candidato está marcado com X e os vizinhos com um circulo. \textit{Retirada de}~\cite{Lowe2004}}
\label{fig:siftdog2}
\end{figure}
%Se esse píxel corresponde a um máximo local é selecionado como candidato a pontos de interesse. Em seguida, são descartados, dentre os candidatos, pontos em arestas e em regiões de pouco contraste da imagem.
...
\subsection{Descritores Locais Binários}
...
%Para além destes dois algoritmos ainda existem outros menos utilizados como o FAST, ORB, BRIEF.... e o HOG (\textit{Histogram of Oriented Gradients})~\cite{Dalal2005}.
